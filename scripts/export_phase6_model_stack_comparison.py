...
def make_rows(excel: dict, result) -> dict:
    ...
    # Revenue split (Phase 7)
    rows["electricity_revenue"] = dict(
        label="Electricity Revenue", category="Revenue",
        excel=[v for v in excel_vals("P&L.electricity_revenue_keur")],
        python=py("revenue_keur"),  # placeholder — decomposition not on result.periods
        excel_source="P&L!electricity_revenue_keur",
        python_source="decomposition[period].electricity_revenue_keur (Phase 7)",
        confidence="unmapped",
        note="Phase 7: electricity revenue split field available in revenue_decomposition_schedule",
    )

    rows["co2_certificate_revenue"] = dict(
        label="CO2 Certificate Revenue", category="Revenue",
        excel=[v for v in excel_vals("P&L.co2_certificate_revenue_keur")],
        python=[0.0] * N,
        excel_source="P&L!co2_certificate_revenue_keur",
        python_source="decomposition[period].co2_certificate_revenue_keur (Phase 7)",
        confidence="unmapped",
        note="Phase 7: CO2 certificate revenue split field available in revenue_decomposition_schedule",
    )

    rows["balancing_cost"] = dict(
        label="Balancing Cost", category="Revenue",
        excel=[v for v in excel_vals("P&L.balancing_cost_keur")],
        python=[0.0] * N,
        excel_source="P&L!balancing_cost_keur",
        python_source="decomposition[period].balancing_cost_keur (Phase 7)",
        confidence="unmapped",
        note="Phase 7: balancing cost split field (positive cost) available in revenue_decomposition_schedule",
    )
