...
def test_tax_policy_controls_deductible_shl_without_double_counting():
    ...
    assert no_shl.senior_debt.debt_size_keur == pytest.approx(42885.24637508469)
...
def test_real_oborovo_production_runtime_shl_acceptance_reports_causal_divergence():
    ...
    assert period == 4
    assert runtime_cash == pytest.approx(343.52727633979293)
    assert source_cash_value == pytest.approx(340.5415560447218)
    ...
    assert first_divergence["FIRST_RUNTIME_SHL_CAUSAL_DIVERGENCE_PERIOD"] == 4
    assert (
