...
class TestOpexStructure:
    ...
    def test_opex_item_escalation(self):
        ...
        # Y1 = 198 kEUR (per Sprint 21 brief calibration — no sub-items)
    ...
    def test_opex_step_change(self):
        ...
        # Y1 = 244 kEUR (per Sprint 21 brief calibration)
        assert abs(infra.amount_at_year(1) - 244.0) < 0.01

        # Y3 = 244 * 1.02^2 ≈ 253.86 (step change, not escalated value)
        assert abs(infra.amount_at_year(3) - 253.86) < 0.01

        # Y4 onwards: continues from base escalation
        y4_expected = 244.0 * (1.02 ** 3)  # no step change, pure escalation
...
class TestFinancingParams:
    ...
    def test_total_equity_shl(self):
        ...
        # Share capital (500) + SHL (13,547.2) + SHL IDC (1,169.0) = 15,216.2 kEUR
        expected = 15216.2
