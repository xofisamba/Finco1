...
class TestBaselinePeriodEngine:
    ...
    def test_operation_period_count(self, engine):
        """Should have 60-61 operation periods (30 years x 2, +/-1 for boundary)."""
        op_periods = [p for p in engine.periods() if p.is_operation]
        assert 59 <= len(op_periods) <= 61, f"Expected 60+/-1, got {len(op_periods)}"

    def test_first_op_period_ends_dec31(self, engine):
        """First operation period ends Dec 31, 2030 after rolling the COD stub."""
        op_periods = [p for p in engine.periods() if p.is_operation]
        assert op_periods[0].end_date.year == 2030
        assert op_periods[0].end_date.month == 12
        assert op_periods[0].end_date.day == 31

    def test_ppa_24_periods(self, engine):
        """PPA should be active for 23-25 periods (12 years x 2 +/-1)."""
