"""Phase 51R-2 — POST /scenarios/{scenario_id}/update-overrides route
family vertical extraction.

This module owns the orchestration body that previously lived
inline in main_web.py for:

- POST /scenarios/{scenario_id}/update-overrides (update_overrides_endpoint)

The route handler in main_web.py is now a thin wrapper
(auth + path param + JSON body parse + deps bundle + service
call + response translation). The full orchestration (scenario
lookup, is_base_case gate, update_scenario_overrides, 500
failure path, partial template re-render with HX-Trigger)
lives here.

This service does NOT import main_web or main_api.
"""
from __future__ import annotations

from dataclasses import dataclass, field
from typing import Any, Callable, Optional


@dataclass
class ScenarioUpdateOverridesRouteOutcome:
    """Result of POST /scenarios/{scenario_id}/update-overrides
    orchestration.

    Two forms:
    - Error (status_code >= 400): JSONResponse(payload, status_code)
    - Success (status_code == 200): template_name + context + headers
    """
    template_name: str = "partials/scenario_tab.html"
    context: dict = field(default_factory=dict)
    payload: dict = field(default_factory=dict)
    status_code: int = 200
    headers: dict = field(default_factory=dict)
    rendered_response: Any = None


@dataclass
class ScenarioUpdateOverridesRouteDeps:
    """Dependencies that ``execute_scenario_update_overrides_route``
    needs from the route.

    The bundle has 6 callables.
    """
    # Scenario lookup
    get_scenario: Callable[..., Any]
    # Persistence (intended write)
    update_scenario_overrides: Callable[..., Any]
    # Workspace state for re-render
    get_project_record: Callable[..., Any]
    get_workspace_state: Callable[..., Any]
    list_scenarios: Callable[..., Any]
    # Render helper
    build_scenario_tab_context: Callable[..., dict]


async def execute_scenario_update_overrides_route(
    *,
    request: Any,
    scenario_id: str,
    overrides: dict,
    user: Any,
    deps: ScenarioUpdateOverridesRouteDeps,
) -> ScenarioUpdateOverridesRouteOutcome:
    """Execute the POST /scenarios/{scenario_id}/update-overrides
    orchestration and return a ScenarioUpdateOverridesRouteOutcome.

    Mirrors the legacy ``update_overrides_endpoint`` route body in
    main_web.py (Phase 51R-1 characterization).
    """
    record = deps.get_scenario(scenario_id, user.user_id)
    if record is None:
        return ScenarioUpdateOverridesRouteOutcome(
            payload={"error": "Scenario not found"},
            status_code=404,
        )
    if record.is_base_case:
        return ScenarioUpdateOverridesRouteOutcome(
            payload={"error": "Cannot override Base Case via this endpoint"},
            status_code=400,
        )

    updated = deps.update_scenario_overrides(user.user_id, scenario_id, overrides)
    if updated is None:
        return ScenarioUpdateOverridesRouteOutcome(
            payload={"error": "Failed to update overrides"},
            status_code=500,
        )

    # Workspace re-render (Quirk 5: partial template, NOT full workspace)
    project_record = deps.get_project_record(
        user_id=user.user_id, project_code=record.project_code
    )
    ws = deps.get_workspace_state(user.user_id, record.project_id)
    scenarios = deps.list_scenarios(
        user.user_id, project_id=record.project_id, include_archived=False, limit=12
    )
    context = deps.build_scenario_tab_context(user, project_record, scenarios, ws)
    return ScenarioUpdateOverridesRouteOutcome(
        template_name="partials/scenario_tab.html",
        context=context,
        status_code=200,
        headers={"HX-Trigger": "overridesUpdated"},
    )
