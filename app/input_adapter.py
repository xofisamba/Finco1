...
def _resolve_user_inputs(
    *,
    ...
    if _scalar_capex:
        ...
        #    The seeded factory is already the default baseline.  However, if a
        #    persisted total_capex_keur differs materially from the factory total,
        #    the project's pre-R1 effective CAPEX was already re-normalised to
        #    that legacy total (zero financial sub-fields + _apply_capex_total).
        #    We must reconstruct that SAME effective baseline before applying the
        #    scalar edit so that untouched lines are not silently reverted to the
        #    pristine seeded factory values.
        #
        # The financing-reset comparison is always vs the pre-edit effective
        # total (total_capex_keur when it differs from factory, else factory).
        _pre_edit_total_for_reset: Optional[float] = None  # computed below
        ...
        if total_capex_keur is not None:
            if base_inputs is None:
                ...
                # pre-edit effective total == total_capex_keur (after normalisation)
                _pre_edit_total_for_reset = float(total_capex_keur)
            else:
                # Case B: seeded project — reconstruct pre-R1 effective state
                # only when the legacy total differs from the seeded factory.
                ...
                if not _legacy_matches_factory:
                    ...
                    proj = _apply_capex_total(proj, total_capex_keur)
                    # Financing was already reset when this divergence was first
                    # persisted (pre-R1 path).  Record the pre-edit effective
                    # total so the reset below triggers on the correct delta.
                    _pre_edit_total_for_reset = float(total_capex_keur)
                    # If frozen schedule was still True before the scalar edit,
                    # it must be reset now (same rule as the legacy path).
        ...
        # change disables the frozen debt schedule (if still set) so the engine
        # can re-size debt from gearing/DSCR inputs.
        if base_inputs is not None:
