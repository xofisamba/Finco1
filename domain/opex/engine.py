...
def compute_annual_opex(
    ...
    *,
    ...
    for group in groups:
        ...
        for y_idx, year in enumerate(year_indices):
            ...
            for item in group.items:
                ...
                # Normal calculation (skip PCT_OF_SELECTED_GROUPS — handled in pass2)
                if item.basis == OpexBasis.PCT_OF_SELECTED_GROUPS:
                    # Skip in pass1 — pct items use final group_totals from pass1
                    # This avoids ordering dependency (pct items can be anywhere in group list)
                    item_results.append(OpexItemAnnualResult(
                        year_index=year,
                        group_code=group.code,
                        group_name=group.name,
                        ...
                        manual_override_keur=None,
                        inflation_rate=eff_infl,
                        wth_rate=item.wth_rate if item.wth_rate > 0 else group.wth_rate,
                        wth_keur=0.0,
                        total_keur=0.0,
                        is_manual_override=False,
                    ))
                    # Note: group_total NOT updated — pct item contributes 0 in pass1
                    continue
    ...
    # ── PASS 2: Compute pct_of_selected_groups items using group totals from Pass 1 ──
    # Process ALL groups (not just contingency_groups) so pct items work even with contingency_pct=0
    for group in groups:
        for y_idx, year in enumerate(year_indices):
            # Find the group result from Pass 1 for this year/group
            pass1_idx = next(
            ...
            # Build updated item list from Pass 1 results
            ...
            for item in group.items:
                ...
                if override is not None:
                    final = override.value_keur
                    wth_keur = final * (item.wth_rate or group.wth_rate)
                    total = final + wth_keur
                    pct_amount += total
                    ...
                    if existing_item_idx is not None:
                        updated_items_list[existing_item_idx] = OpexItemAnnualResult(
                            ...
                            calculated_keur=0.0,
                            manual_override_keur=override.value_keur,
                            inflation_rate=item.inflation_rate or group.inflation_rate,
                            wth_rate=item.wth_rate or group.wth_rate,
                            wth_keur=wth_keur,
                            total_keur=total,
                            is_manual_override=True,
                        )
                ...
                base = selected_total * item.budget_keur / 100.0
            ...
            # For groups with contingency_pct > 0, pct amount IS the contingency.
            # For groups with contingency_pct == 0, pct amount is just normal revenue (no contingency label).
...
def _compute_item_base(
    ...
    if item.basis == OpexBasis.PCT_OF_SELECTED_GROUPS:
        ...
        # selected_group_codes = list of group codes to sum
        selected_total = sum(
            group_totals[gc][y_idx]
            for gc in item.selected_group_codes
            if gc in group_totals
