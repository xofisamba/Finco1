...
B02_EXPLICIT_SCHEDULE = [
    ...
    628.0, 628.0, 628.0, 628.0, 628.0,  # Y11, Y12, Y13, Y14, Y15
    676.0, 676.0, 676.0, 676.0, 676.0,  # Y16, Y17, Y18, Y19, Y20
    756.0, 756.0, 756.0, 756.0, 756.0,  # Y21, Y22, Y23, Y24, Y25
    828.0, 828.0, 828.0, 828.0, 828.0,  # Y26, Y27, Y28, Y29, Y30
]
...
def _i(code: str, name: str, budget: float,
    ...
    return OpexItem(
        ...
        active_flags=active_flags, explicit_schedule_keur=list(explicit) if explicit else [],
...
def build_tuho_opex_template() -> list[OpexGroup]:
    ...
    Acceptance criteria:
    - build_tuho_opex_template() works
    - compute_annual_opex with TUHO template gives: Excel total = 84,674.78 kEUR
    - max annual delta < ±0.01 kEUR

    Notes
    -----
    - B.02 group inflation_rate = 0.0 (explicit schedule item not inflated)
    - B.11 uses explicit 30-element active_flags (Y1–Y14 active, Y15–Y30 inactive)
