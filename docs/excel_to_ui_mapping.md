...
Excel export integration is **not implemented** in this sprint. The `build_excel_export()` function in `app/excel_export.py` is not modified. A future sprint should add an optional "OPEX Detail" tab when advanced OPEX is enabled.

### Next Steps

1. Add add/delete line item functionality to the UI
2. Excel export OPEX Detail tab
3. CAPEX line-item model (mirrors OPEX structure after OPEX stabilises)
4. Support BESS/Hybrid advanced OPEX
