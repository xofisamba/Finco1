...
6. **Is PPA Tariff runtime-bound?** → YES (`tariff_eur_mwh` is passed to `build_projectinputs` → `RevenueInput`)
...
Output Preview placeholder directs users to the runtime summary location after running.
